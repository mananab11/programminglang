...
def p_program_block(p):
    'program : function_stmt LBRACE statements RBRACE'
    # print("in program")
    p[0]=p[1]+p[3]

def p_functions_nonempty(p):
    'function_stmt : function function_stmt'
    # print("in functions")
    p[0] = [p[1]] + p[2]

def p_functions_empty(p):
    'function_stmt :'
    # print("in empty func",p[0])
    p[0] = []

def p_function_withnoargs(p):
    'function : FUN IDENTIFIER LPAREN RPAREN EQUALS statement expression SEMICOLON'
    p[0] = tree.Function_n(p[2], (), p[6], p[7])

def p_function_withargs(p):
    'function : FUN IDENTIFIER LPAREN IDENTIFIER args RPAREN EQUALS statement expression SEMICOLON'
    # print("in function grammar")
    p[0] = tree.Function_N(p[2], (p[4],) + p[5], p[8], p[9])

def p_expr_functioncall_withnoargs(p):
    'expression : IDENTIFIER LPAREN RPAREN'
    p[0] = tree.CallFunction_N(p[1], ())

def p_expr_functioncall_withargs(p):
    'expression : IDENTIFIER LPAREN expression exprtail RPAREN'
    # print("in function call grammar")
    p[0] = tree.CallFunction_N(p[1], (p[3],)+p[4] )

def p_expressiontail_nonempty(p):
    'exprtail : COMMA expression exprtail'
    p[0] = (p[2],) + p[3]


def p_expressiontail_empty(p):
...
def p_statement_expr(p):
    'statement : expression EQUALS expression SEMICOLON'
    # print("i am in assign grammar")
    # print(p[1],p[2],p[3])
    # p[1]=IdentiferN(p[1])
